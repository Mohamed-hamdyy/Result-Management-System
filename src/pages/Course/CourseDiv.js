...
export function CourseDiv (props) {
  ...
  return (
    ...
    <div className={courseDetails ? 'newCourse-After' : 'newCourse'}>
      ...
      <BiDownArrow className='icon' style={{ marginRight: '1rem', transform: 'translate(0 ,0.4rem)' }} onClick={handleCourseDetails} />
      <button
        className='openCourse'
        onClick={() => {
          if (!props.inst) {
            navigate('/TraineeCourse', { state: { id: props.course.id, View: 'Overview' } })
          } else {
